...
class ConcreteBuilder1(Builder):
    ...
    def product(self) -> Product1:
        ...
        Therefore, such methods cannot be declared in the base Builder interface (at least in a statically typed
        programming language).
        Usually, after returning the end result to the client, a builder instance is expected to be ready to start
        producing another product. That's why it's an usual practice to call the reset method at the end of the `product`
        method body. However, this behavior is not mandatory, and you can make your builders wait for an explicit reset call
...
class Director():
    """
    The Director is only responsible for executing the building steps in a particular sequence. It is helpful when producing
    products according to specific order or configuration. Strictly speaking, the Director class is optional, since the client
    can control builders directly.
    """
    def __init__(self) -> None:
        self._builder = None
...
if __name__ == "__main__":
    """
    The client code creates a builder object, passes it to the director and then iniates the construction process.
    The end result is retrieved from the build object
    """
    ...
    # Remember, the Builder can be used without a Director class.
